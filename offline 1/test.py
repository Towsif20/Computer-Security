# file = open("AES Overview.pptx", "rb")
#
# while True:
#     txt = file.read(16)
#     if not txt:
...
# print(g(["67", "20", "46", "75"], "01"))

# def make_round_key(round):


# print(matrix_maker(string))
...
# output_file.write(bytearray(results))
...
for ciph in results:
    for i in range(4):
        for j in range(4):
            c = chr(int(BitVector(hexstring=ciph[j][i])))
            # print(c, end='')
            # output_file.write(bytes(ciph[j][i]))
            output_file.write(bytearray(c.encode('utf-8')))
